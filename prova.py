...
def write_text(text,name, time_sleep=0.4, first_sleep = 1, speaker=True):
...
def on_click_name(text, name, time_sleep=0.1, answer = False):
    if not answer:
...
def main():
    ...
    #    ask = st.button("Ask me something 🔊")
        ask = st.text_input("Ask me something 🔊")
        text = ask
    #    r = sr.Recognizer()
    #    if ask:
    ...
    #                st.error("Sorry, I didn't understand what you said.")
        if text != '' and st.session_state.name is not None:
            gen_text = generate_text(text,st.session_state.name)
            #write_text(text,name='You', time_sleep=0.3, first_sleep=0.1, speaker=False)                  
            on_click_name(gen_text, st.session_state.name, answer=True, time_sleep=0.05)
        elif text != '' and st.session_state.name is None:
            st.error("Please select a character first!")
